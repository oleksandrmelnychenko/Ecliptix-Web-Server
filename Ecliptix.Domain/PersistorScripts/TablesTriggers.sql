...
-- Таблиця AuthenticationContexts: Зберігає контексти аутентифікації (замість сесій)
CREATE TABLE dbo.AuthenticationContexts (
    Id                  BIGINT IDENTITY(1,1) PRIMARY KEY,
    ContextToken        VARBINARY(64) NOT NULL UNIQUE, -- Токен контексту аутентифікації
    MembershipId        UNIQUEIDENTIFIER NOT NULL, -- Посилання на Memberships.UniqueId
    MobileNumberId      UNIQUEIDENTIFIER NOT NULL, -- Посилання на PhoneNumbers.UniqueId (змінено з PhoneNumberId)
    ...
    IpAddress           NVARCHAR(45), -- IPv4/IPv6 адреса
    UserAgent           NVARCHAR(500), -- User Agent клієнта
    ...
    CONSTRAINT FK_AuthContexts_Memberships FOREIGN KEY (MembershipId) REFERENCES dbo.Memberships(UniqueId) ON DELETE CASCADE,
    CONSTRAINT FK_AuthContexts_MobileNumbers FOREIGN KEY (MobileNumberId) REFERENCES dbo.PhoneNumbers(UniqueId) ON DELETE NO ACTION
);
CREATE NONCLUSTERED INDEX IX_AuthContexts_ContextToken ON dbo.AuthenticationContexts (ContextToken); -- Індекс для швидкого пошуку за токеном
CREATE NONCLUSTERED INDEX IX_AuthContexts_MobileNumberId_Active ON dbo.AuthenticationContexts (MobileNumberId, IsActive); -- Індекс для пошуку активних контекстів
CREATE NONCLUSTERED INDEX IX_AuthContexts_ExpiresAt ON dbo.AuthenticationContexts (ExpiresAt) WHERE IsActive = 1; -- Індекс для очищення прострочених
GO

-- Таблиця AuthenticationStates: Зберігає стани аутентифікації для оптимізації (in-memory backup)
CREATE TABLE dbo.AuthenticationStates (
    MobileNumberId      UNIQUEIDENTIFIER PRIMARY KEY, -- Посилання на PhoneNumbers.UniqueId
    RecentAttempts      INT NOT NULL CONSTRAINT DF_AuthStates_RecentAttempts DEFAULT 0, -- Кількість останніх спроб
    WindowStartTime     DATETIME2(7) NOT NULL CONSTRAINT DF_AuthStates_WindowStartTime DEFAULT GETUTCDATE(), -- Початок вікна rate limiting
    LastAttemptTime     DATETIME2(7), -- Час останньої спроби
    IsLocked            BIT NOT NULL CONSTRAINT DF_AuthStates_IsLocked DEFAULT 0, -- Чи заблокований
    LockedUntil         DATETIME2(7), -- Заблокований до якого часу
    LastSyncTime        DATETIME2(7) NOT NULL CONSTRAINT DF_AuthStates_LastSyncTime DEFAULT GETUTCDATE(), -- Час останньої синхронізації з пам'яттю
    ...
    CONSTRAINT FK_AuthStates_MobileNumbers FOREIGN KEY (MobileNumberId) REFERENCES dbo.PhoneNumbers(UniqueId) ON DELETE CASCADE
);
CREATE NONCLUSTERED INDEX IX_AuthStates_IsLocked_LockedUntil ON dbo.AuthenticationStates (IsLocked, LockedUntil); -- Індекс для пошуку заблокованих
GO

...
public class ProtocolHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            // Check if ActorSystem is running
            if (_actorSystem.WhenTerminated.IsCompleted)
            {
                return HealthCheckResult.Unhealthy("ActorSystem is terminated");
            }
            
            // Check DH ratchet frequency
            ...
            {
                foreach (var conn in _connectionHealth.Values)
                {
                    if (conn.IsHealthy)
                        healthyConnections++;
                    else
    ...
    // Methods called by actors to update metrics
    public static void RecordDhRatchet(uint connectId, bool success)
    {
        ...
        lock (_connectionHealth)
        {
            if (!_connectionHealth.TryGetValue(connectId, out var health))
            {

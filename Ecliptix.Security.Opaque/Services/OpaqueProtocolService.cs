...
public sealed class OpaqueProtocolService : INativeOpaqueProtocolService, IDisposable
{
    ...
    public async Task<Result<Unit, OpaqueServerFailure>> InitializeAsync(string? secretKeySeed = null) =>
        await Task.Run(() => Initialize(secretKeySeed));
        ...
        try
        {
            ...
            else
            {
                ...
                if (result != OpaqueResult.Success)
                    return Result<Unit, OpaqueServerFailure>.Err(
                        OpaqueServerFailure.LibraryInitializationFailed(
                            $"{OpaqueServerConstants.ErrorMessages.FailedToCreateServer}: {result}"));
            }
    ...
    public async Task<Result<(RegistrationResponse Response, byte[] ServerCredentials), OpaqueServerFailure>>
        CreateRegistrationResponseAsync(RegistrationRequest request)
    {
        await Task.Yield();

        if (!_isInitialized)
            return Result<(RegistrationResponse, byte[]), OpaqueServerFailure>.Err(OpaqueServerFailure
            ...
            if (request?.Data == null || request.Data.Length == 0)
                return Result<(RegistrationResponse, byte[]), OpaqueServerFailure>.Err(
                    OpaqueServerFailure.InvalidInput(OpaqueServerConstants.ValidationMessages
                        .RegistrationRequestDataRequired));
    ...
    public async Task<Result<SessionKey, OpaqueServerFailure>> FinishAuthenticationAsync(KE3 ke3)
    {
        ...
        try
        {
            ...
            if (ke3?.Data == null || ke3.Data.Length == 0)
                return Result<SessionKey, OpaqueServerFailure>.Err(
                    OpaqueServerFailure.InvalidInput(OpaqueServerConstants.ValidationMessages.KE3DataRequired));
    ...
    private static Result<DerivedServerKeys, OpaqueServerFailure> DeriveKeysFromSeed(string seed)
    {
        try
        {
